    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (!branchExists) {
                ...
                yield generateBranch(constants_1.action, repositoryPath);
            }
            ...
            yield util_1.execute(`git add -f ${constants_1.build}`, constants_1.workspace);
            yield util_1.execute(`git commit -m "Deploying to ${constants_1.action.branch} from ${constants_1.action.baseBranch || 'master'} ${process.env.GITHUB_SHA}"`, constants_1.workspace);
            yield util_1.execute(`git push ${repositoryPath} \`git subtree split --prefix ${constants_1.build} ${constants_1.action.baseBranch || 'master'}\`:${constants_1.action.branch} --force`, constants_1.workspace);
        }
        catch (error) {
